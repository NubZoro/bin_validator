# app.py
# Streamlit app with integrated model functionality
...
def load_model_resources():
    """
    load groundingdino + CLIP + FAISS resources.
    ...
    # grounding dino config with checkpoint paths
...
def run_model_on_image(image_path: str):
    """
    Run the detection with matching with counting pipeline on a single image path.
    Returns a dict
    """
    try:
...
with main_col:
    ...
    if st.session_state.page == "Order Validation":
        ...
        # Validate button and allocation
