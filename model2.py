...
# Returns list of image files sorted alphabetically
def list_imgs(d):
    return sorted([p for p in glob.glob(os.path.join(d, "*")) if p.lower().endswith((".jpg", ".png", ".jpeg"))])

# Fetch metadata JSON based on filename
...
# Converts product metadata into a readable descriptive sentence for CLIP text embeddings
...
# Converts list of texts into CLIP embeddings
...
# Convert PIL image into CLIP embeddings
...
# Build FAISS similarity index for efficient nearest-neighbor search
...
if not os.path.exists(CONFIG_PATH):
    print("Downloading Grounding DINO config file")
...
def detect_regions_dino(img, conf=0.25, max_det=400):
    ...
    # Run the model with caption 'object'
...
def match_regions_to_asins(img: Image.Image, boxes_scored, asin_texts, sim_th=0.20):
    """
    For each region detected by DINO:
      - Crop the region
      - Encode crop with CLIP
      - Compare with FAISS index of ASIN text embeddings
    """
    if not asin_texts:
        return []
